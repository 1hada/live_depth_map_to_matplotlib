...
// csv stuff
bool save_csv_now = false ;
int file_count = 1 ; 
std::string file_path = "/home/biobe/workspace/";
std::string f_with_prefix = file_path+"point_cloud_";
std::ofstream myfile; 
/*
struct point_cloud {
    float x;
...
void DrawGLScene()
{
    ...
    //static std::vector<point_cloud> pc(640*480);
    float x, y, z, r, g , b, a ;
    a = 1;
    
    if(save_csv_now){
        std::string file_name = f_with_prefix+std::to_string(file_count)+".csv";
        myfile.open(file_name); //file opening constructor;
        ++file_count;
    }
    ...
    {   
        ...
        x = (i%640 - (640-1)/2.f) * depth[i] / f ;// X = (x - cx) * d / fx
        y = (i/640 - (480-1)/2.f) * depth[i] / f ;// Y = (y - cy) * d / fy
        z = depth[i] ; // Z = d
        //pc.push_back({x, y, z});
        glVertex3f( x, y, z);                           
        if(save_csv_now){
            myfile <<x<<","<<y<<","<<z<<","<<r<<","<<g<<","<<b<<","<<a<< std::endl;
        }
    }
    if(save_csv_now){
        myfile.close();
        std::cout << "file save OK" << std::endl;
        save_csv_now = false;
    }
    glEnd();
...
void keyPressed(unsigned char key, int x, int y)
{
    switch (key)
    {
        ...
        case  's':
            ...
            if(!save_csv_now) save_csv_now = true;
            break;
        case  'Q':
        case  'q':
        case 0x1B:  // ESC
